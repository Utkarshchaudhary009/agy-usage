...
CREATE POLICY "Users can manage their own wakeup configs" ON public.wakeup_configs
  FOR ALL TO authenticated USING (clerk_user_id = requesting_user_id());

CREATE POLICY "Users can read their own wakeup logs" ON public.wakeup_logs
  FOR SELECT TO authenticated USING (clerk_user_id = requesting_user_id());

CREATE POLICY "Users can insert their own wakeup logs" ON public.wakeup_logs
  FOR INSERT TO authenticated WITH CHECK (clerk_user_id = requesting_user_id());